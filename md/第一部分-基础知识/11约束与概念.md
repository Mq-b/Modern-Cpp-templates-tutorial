...
**析构函数比较特殊**，不需要我们显式声明它为 `noexcept` 的；

不管编译器为我们生成的，还是我们用户显式定义的 `~X()` 析构函数，默认都是有 `noexcept` 的。只有我们用户定义析构函数的时候为它修饰了 `noexcept(false)` 才会不满足 *概念*（concept）`C2` 的要求。参见[文档](https://zh.cppreference.com/w/cpp/language/destructor#:~:text=%E5%A6%82%E6%9E%9C%E6%B2%A1%E6%9C%89%E6%98%BE,C%2B%2B11%20%E8%B5%B7))。

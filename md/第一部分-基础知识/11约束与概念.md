...
> 三端[测试](https://godbolt.org/z/svdTnonbf)。
...
> 以上代码利用了实参依赖查找（`ADL`），即 `swap(X{})` 是合法表达式，而不需要增加明明空间限定。
...
3. 如果*返回类型要求* 存在，则：
   a) 模板实参被替换到*返回类型要求* 中；
   b) `decltype((表达式))` 必须满足*类型约束* 蕴含的约束。否则，被包含的 requires 表达式是 **`false`**。
...
concept C3 = requires(T a, std::size_t n) {
    requires std::is_same_v<T*, decltype(&a)>;     // 嵌套要求，要求 is_same_v 求值为 true
    requires std::same_as<T*, decltype(new T[n])>; // 嵌套要求，要求 same_as   求值为 true
    requires requires{ a + a; };                   // 嵌套要求，要求 requires{ a + a; } 求值为 true
};
std::cout <<std::boolalpha<< C3<int> << '\n'; // true
```

> 嵌套要求的 *约束表达式*，只要能编译期产生 `bool` 值的表达式即可，不管是*概念*（concept）、[类型特征](https://zh.cppreference.com/w/cpp/meta#.E7.B1.BB.E5.9E.8B.E7.89.B9.E5.BE.81)的库、`requires` 表达式，都一样。

...
2. 为什么需要 T1{}，T2{} 这种形式？没有办法，必须构造临时对象来写成这种形式，这里其实是不求值语境，我们只是为了写出这样一种形式，让编译器获取类型罢了。
...
那么不能模板不能分文件[^3]的原因就显而易见了，我们在讲[使用模板](#使用模板)的时候就说了：
...
[^3]: 注：这个问题可以通过显式实例化解决，在[后面](06模板显式实例化解决模板分文件问题.md)会讲。